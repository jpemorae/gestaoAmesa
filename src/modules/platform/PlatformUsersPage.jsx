    ...
    <section className="content">
      <section className="panel compact-panel">
        <div className="panel-title-row">
          <h2>{editingUserId ? "Editar usuário master" : "Cadastrar usuário master"}</h2>
        ...
        <form className="form-grid dense-form" onSubmit={onSave}>
          <TextField label="Nome" value={userForm.name} placeholder="Nome do usuário master" onChange={(name) => onUserFormChange({ ...userForm, name })} />
          ...
          <button className="primary" type="submit">{editingUserId ? "Salvar alterações" : "Salvar usuário master"}</button>
        </form>
      </section>

      <section className="panel compact-panel">
        <h2>Usuários master cadastrados</h2>
        {users.length === 0 ? <div className="empty">Nenhum usuário master cadastrado.</div> : (
